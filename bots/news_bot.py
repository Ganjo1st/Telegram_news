...
class NewsBot:
    def __init__(self):
        logger.info("🔄 Инициализация NewsBot...")
    ...
    def _parse_infobrics_article(self, url: str) -> dict | None:
        try:
            logger.info(f"🔍 Парсинг InfoBrics статьи: {url[:60]}...")
            resp = fetch_url(url)
            if not resp:
            ...
            self.total_found += 1
            logger.info(f"✅ InfoBrics: статья прошла проверку: {title[:50]}...")
    ...
    def _parse_globalresearch_article(self, url: str) -> dict | None:
        try:
            logger.info(f"🔍 Парсинг Global Research статьи: {url[:60]}...")
            resp = fetch_url(url)
            if not resp:
                logger.warning(f"❌ Не удалось загрузить страницу: {url}")
                return None
            ...
            self.total_found += 1
            logger.info(f"✅ Global Research: статья прошла проверку: {title[:50]}...")
    ...
    def _parse_presstv_article(self, url: str) -> dict | None:
        try:
            logger.info(f"🔍 Парсинг Press TV статьи: {url[:60]}...")
            resp = fetch_url(url)
            if not resp:
                logger.warning(f"❌ Не удалось загрузить страницу: {url}")
                return None
            ...
            self.total_found += 1
            logger.info(f"✅ Press TV: статья прошла проверку: {title[:50]}...")
    ...
    async def fetch_news(self) -> list:
        ...
        for article in ib_articles[:7]:
            ...
            if data and not self._is_duplicate(article['url'], article['title'], data['content']):
                items.append(data)
                logger.info(f"✅ InfoBrics добавлена: {data['title'][:50]}...")
        ...
        for article in gr_articles[:5]:
            ...
            if data and not self._is_duplicate(article['url'], article['title'], data['content']):
                items.append(data)
                logger.info(f"✅ Global Research добавлена: {data['title'][:50]}...")
        ...
        for article in pt_articles[:5]:
            ...
            if data and not self._is_duplicate(article['url'], article['title'], data['content']):
                items.append(data)
                logger.info(f"✅ Press TV добавлена: {data['title'][:50]}...")
...
async def main():
    ...
    try:
        ...
        if 'GITHUB_ACTIONS' in os.environ:
            logger.info("📌 Запуск в GitHub Actions (один раз)")
            await bot.run_once()
        else:
            logger.info("📌 Запуск в локальном режиме (бесконечный цикл)")
